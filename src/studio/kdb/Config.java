...
public class Config {
    ...
    private Properties p = new PropertiesConfig();
    ...
    public void saveWorkspace(Workspace workspace) {
        try {
            Properties p = new PropertiesConfig();
            workspace.save(p);
            OutputStream out = filesBackup.newFileOutputStream(getWorkspaceFilename());
    ...
    private void init(String filename) {
        filesBackup = new FilesBackup(EnvConfig.getFilepath(BACKUP_FOLDER));
        load(filename);
        checkForUpgrade();
        initTableConnExtractor();
    }
    ...
    public void save() {
        try {
            OutputStream out = filesBackup.newFileOutputStream(filename);
    ...
    public String[] addServers(boolean tryAll, Server... newServers) {
        ...
        Properties backup = new PropertiesConfig();
        backup.putAll(p);
        try {
            int index = -1;
